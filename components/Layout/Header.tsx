...
const Header: React.FC<HeaderProps> = ({ toggleSidebar, title, selectedClass }) => {
  return (
    <header className="sticky top-0 z-10 bg-gray-50/90 backdrop-blur-md px-6 md:px-10 py-5 flex items-center justify-between transition-all duration-300 shadow-sm border-b border-gray-100/50">
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="flex flex-col md:flex-row md:items-baseline gap-1 md:gap-4 overflow-hidden">
            <h1 className="font-display text-2xl md:text-3xl font-bold text-navy-900 whitespace-nowrap">{title}</h1>
            
            <div className="text-xs md:text-sm font-medium text-gray-500 flex flex-col md:flex-row md:items-center md:gap-3 leading-tight truncate">
                {selectedClass === 'MW' ? (
                    <>
                        <span className="flex items-center gap-1.5 shrink-0">
                            <span className="w-2 h-2 rounded-full bg-brand-500 animate-pulse"></span>
                            <span className="text-brand-600 font-bold">월수반</span>
                        </span>
                    ...
                    <>
                        <span className="flex items-center gap-1.5 shrink-0">
                            ...
                            <span className="text-green-600 font-bold">화목반</span>
                        </span>
                        <span className="hidden md:block w-px h-3 bg-gray-300"></span>
                        <span className="truncate">
